...
func _physics_process(delta):
	...
	if is_on_floor():                                              #interactions for if the player is on the floor
		if not moving:                                             #if the player is not moving, reduce velocity using friction
			...
			if abs(velocity.x) < 10:                               #if player velocity is less than 10, set to 0, this is so that the player doesnt slide an infinitely small amount infinitely
				velocity.x = 0
	else:                                                          #if the player is not on the floor, add air_friction
	...
	# Jumping stuff
	if Input.is_action_just_pressed("Jump") and jumps > 0:                               #will run if the player has more than 0 jumps and pressed jump, this checks if the player has MORE THAN 0 jumps, this is important because if the value somehow becomes negative, the player could infinitely jump
			
			if is_on_wall():                                                              #if the player is touching a wall, set jumps to max, effectively reseting their amount of jumps
			...
			elif jumps == max_jumps:                                                      #only runs if player is not on a wall, checks if the jump is the first jump, this is for using different animations for double and normal jumps
				velocity.y = -jumpForce
				$Smoothing2D/AnimatedSprite2D.play("Jump")
			else:                                                                         #runs when the player has double jumped
